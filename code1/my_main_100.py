...
def main():
    ...
    parser.add_argument("--data_dir",
                        default='../data/琼海市塔洋镇卫生院',
                        type=str,
                        required=False,
                        help="The input data dir.")
    parser.add_argument("--pretrain_dir", default='../saved/best', type=str, required=False,
    ...
    for i in range(len(train_dataloader_list)):
        ...
        for k in range(0,100):
            model_env.train()
            train_loss = model_rlkg.fit2(train_dataloader=train_dataloader_list[i],device=device)
            rx_save_path = './result/train/'+today+"/"+str(i)
            if not os.path.isdir(rx_save_path):
                os.makedirs(rx_save_path)
            my_utils.write_txt('./train_loss_simre.txt', str(train_loss))
            model_env.eval()
            ...
            my_utils.write_txt('./eval_loss_simre.txt', str(eval_loss))
