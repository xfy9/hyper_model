...
def expand_level2():
    ...
    level2 =['A00-B99','C00-D48','D50-D89','E00-E90','F00-F99','G00-G99','H00-H59','H60-H99',
             'I00-I99','J00-J99','K00-K93','L00-L99','M00-M99','N00-N99','O00-O99','P00-P96','Q00-Q99','R00-R99',
             'S00-T98','U00-U99','V01-Y98','Z00-Z99']
    level2_expand = {}
    for i in level2:
        tokens = i.split('-')
        if i[0] == 'H':
            if len(tokens) == 1:
                ...
                for j in range(int(tokens[0][1:]), int(tokens[1][1:]) + 1):
                    level2_expand["H%02d" % j] = i
        elif i[0] != i[4]:
            for j in range(int(tokens[0][1:]), 100):
                level2_expand[i[0] + "%02d" % j] = i
            for j in range(int(tokens[0][1:]), int(tokens[1][1:]) + 1):
                level2_expand[i[4] + "%02d" % j] = i
    ...
    for code in unique_codes:
        ...
        if '.' in code:
            ...
            if code.isalnum():
                # if level2 == 'BNF010':
                #     print("hello")
                level2=re.findall(r'[A-Za-z]+|\d+', code)[0]
                level3 = level2[0:2]
        # level2=code.split('.')[0]
        # if level2=='BNF010':
        #     continue
...
def build_atc_tree(unique_codes):
    ...
    root_node = 'atc_root'
    for code in unique_codes:
        sample = [code] + [code[:i] for i in [13, 8, 3 , 2]] + [root_node]
...
def build_sym_tree(unique_codes):
    res = []
    graph_voc = Voc()

    root_node = 'sym_root'
    for code in unique_codes:
        sample = [code] + [code[:1] ] + [root_node]
