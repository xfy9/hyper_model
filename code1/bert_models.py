...
class SublayerConnection(nn.Module):
    ...
    Note for code1 simplicity the norm is first as opposed to last.
    """
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 词嵌入编码， 第一个参数为出现的单词总数，第二个参数为希望多少维的向量表示一个单词
        # 编码出现的所有单词
...
class BERT(PreTrainedBertModel):
    ...
    def __init__(self, config: BertConfig, body_tem_voc=None,pulse_rate_voc=None,breathing_rate_voc=None,lsbp_voc=None,ldbp_voc=None):
        """
        ...
        if config.graph:
            assert body_tem_voc is not None
            assert pulse_rate_voc is not None

        # embedding for BERT, sum of positional, segment, token embeddings
        self.embedding = BertEmbeddings(config)
        self.bertEmbeddings = BertEmbeddings(config)
    ...
    def forward(self, x, token_type_ids=None, input_positions=None, input_sides=None):
        ...
        # 扩展张量第一个维度，变为三维tensor，原始第一个维度现在第二个维度
        # x-> torch.Size([4, 55])  mask ->torch.Size([4, 1, 55, 55])
        mask = (x > 1).unsqueeze(1).repeat(1, x.size(1), 1).unsqueeze(1)

        # embedding the indexed sequence to sequence of vectors
        # token_type_ids -> torch.Size([4, 55])
        ...
        # for transformer in self.transformer_blocks:
        #     x = transformer.forward(x, mask)
        # # x -> torch.Size([4, 55, 300]) batch hidden_size
        # return x, x[:, 0]
